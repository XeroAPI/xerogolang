...
func (p *Provider) FetchUser(session goth.Session) (goth.User, error) {
	...
	var organisationResponse model.OrganisationResponse
	err = json.Unmarshal(responseBytes, &organisationResponse)
	if err != nil {
		return user, fmt.Errorf("Could not unmarshal response: %s", err.Error())
	}

	user.Name = organisationResponse.Organisations[0].Name
	user.NickName = organisationResponse.Organisations[0].LegalName
	user.Location = organisationResponse.Organisations[0].CountryCode
	user.Description = organisationResponse.Organisations[0].OrganisationType
	user.UserID = organisationResponse.Organisations[0].ShortCode
...
func (p *Provider) Find(session goth.Session, endpoint string) ([]byte, error) {
	...
	if sess.AccessToken == nil {
		...
		return nil, fmt.Errorf("%s cannot get user information without accessToken", p.providerName)
	}
	...
	if response.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("%s responded with a %d trying to fetch user information: %s", p.providerName, response.StatusCode, response.Body)
	}
